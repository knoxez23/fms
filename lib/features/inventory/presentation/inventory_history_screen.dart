...
class _InventoryHistoryViewState extends State<InventoryHistoryView> {
  ...
  _InventoryHistoryData _buildHistoryFromItems(List<InventoryItem> items) {
    ...
    final usageHistory = allItems.where((i) {
      final q = (i['quantity'] as num?)?.toDouble() ?? 0;
      return q > 0;
    }).map((i) {
      final quantity = (i['quantity'] as num?)?.toDouble() ?? 0;
      final minStock = (i['minStock'] as num?)?.toDouble() ?? 0;
      final impliedUsage = minStock > 0
          ? (minStock - quantity).clamp(0, double.infinity)
          : quantity * 0.15;
      final dateRaw = (i['lastRestock'] as String?) ?? '';

      return {
        'item': i['name'],
        'quantity': impliedUsage.toDouble(),
        'unit': i['unit'],
        'purpose': minStock > 0 && quantity <= minStock
            ? 'Low stock correction'
            : 'Routine usage estimate',
        'date': dateRaw,
      };
    }).toList();

    final totalRestockCost = restockHistory.fold<double>(
      0,
...
class _UsageTab extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final monthly = _aggregateMonthlyUsage(usageHistory);
    return ListView(
      padding: const EdgeInsets.all(16),
      children: [
        _AnimatedCard(
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                if (monthly.isEmpty)
                  Text(
                    'Not enough dated records to render chart',
                    style: theme.textTheme.bodySmall?.copyWith(
                      color: theme.colorScheme.onSurface.withValues(alpha: 0.6),
                    ),
                  )
                else
                  _UsageBarChart(monthly: monthly, theme: theme),
              ],
            ),
          ),
        ),
  ...
  Map<String, double> _aggregateMonthlyUsage(
    ...
    for (final row in rows) {
      ...
      totals[key] =
          (totals[key] ?? 0) + ((row['quantity'] as num?)?.toDouble() ?? 0);
    }
...
class _UsageBarChart extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final maxValue = monthly.values.fold<double>(0, (m, v) => v > m ? v : m);
    ...
    return SizedBox(
      ...
      child: Row(
        ...
        children: monthly.entries.map((entry) {
          final ratio = (entry.value / safeMax).clamp(0.08, 1.0);
          return Expanded(
            child: Padding(
