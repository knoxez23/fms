...
import 'package:pamoja_twalima/features/farm_mgmt/presentation/bloc/crops/crops_bloc.dart';
    ...
    return BlocProvider(
      ...
      child: BlocConsumer<CropsBloc, CropsState>(
        ...
        builder: (context, state) {
          ...
          final readyCount = crops.where((c) => c.isReadyForHarvest).length;
          final avgArea = crops.isEmpty
              ? 0.0
              : crops
                      .map((c) => c.area ?? 0)
                      .reduce((a, b) => a + b) /
                  crops.length;
          final withExpectedHarvest =
              crops.where((c) => c.expectedHarvest != null).length;
          final harvestProgress = crops.isEmpty
              ? 0
              : ((withExpectedHarvest / crops.length) * 100).round();
                  ...
                  sliver: SliverToBoxAdapter(
                    child: Row(
                      children: [
                        ...
                        _CropStat(
                          value: avgArea.toStringAsFixed(1),
                          label: 'Avg Area (acres)',
                          theme: theme,
                        ),
                        const SizedBox(width: 16),
                        _CropStat(
                          value: '$readyCount/$harvestProgress%',
                          label: 'Ready/Tracked',
                          theme: theme,
                        ),
                      ],
    ...
    return Card(
      ...
      child: ListTile(
        ...
        subtitle: Column(
          ...
          children: [
            Text(
              'Type: ${crop.variety ?? crop.name.value} • Area: ${crop.area?.toStringAsFixed(1) ?? 'Not set'} acres',
            ),
            Text(
              'Status: ${crop.status ?? (crop.isReadyForHarvest ? 'Ready' : 'Growing')}',
              style: theme.textTheme.bodySmall?.copyWith(
                color: theme.colorScheme.onSurface.withValues(alpha: 0.6),
              ),
            ),
